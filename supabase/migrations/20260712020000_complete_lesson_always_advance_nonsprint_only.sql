-- ============================================================================
-- 20260712020000_complete_lesson_always_advance_nonsprint_only.sql
-- Scope the "always auto-advance on completion" behavior (added in
-- 20260712010000) to NON-Sprint packs only, so current production is not
-- affected.
--
-- WHY: every current production user is on the Sprint pack
--   (b0000000-0000-0000-0000-000000000001) — the only production_ready pack.
--   The always-advance change must not alter that live experience. James and
--   all future coach packs only become prod-visible with the app update, so
--   scoping always-advance to non-Sprint packs means the new behavior is
--   effectively dormant until those packs go live.
--
-- BEHAVIOR:
--   Sprint (b0000000-...-0001): advance ONLY on a new (non-duplicate)
--     completion of the current-day lesson — identical to the pre-20260712010000
--     production behavior. Current prod is unchanged.
--   All other packs (James + future): advance whenever the current-day lesson
--     is completed, including same-day duplicates (the fix the user asked for).
--
-- Still self-guarded by v_is_current_day_lesson, so no pack can double-advance.
-- MAC scoring and streaks remain gated on `not v_is_duplicate` (unchanged).
--
-- TO LATER APPLY ALWAYS-ADVANCE TO SPRINT TOO: change the advance condition
--   below to just `if v_is_current_day_lesson then` (drop the Sprint carve-out).
--
-- Signature and returned jsonb shape unchanged. Supersedes 20260712010000.
-- KEEP IN SYNC WITH supabase/functions/_shared/scoring.ts (gain steps / decay).
...
declare
  ...
  -- Program-switching additions:
  v_active_program_id       uuid;
  v_current_day             int;
  v_is_current_day_lesson   boolean := false;
  v_total_days              int := 0;
  v_new_day                 int;
  -- Sprint is the current live prod pack; its advance behavior must not change.
  v_sprint_program_id       uuid := 'b0000000-0000-0000-0000-000000000001';
begin
  ...
  -- ====================================================================
  -- Program day advance (program_day, last_wod, per-pack state mirror).
  -- Non-Sprint packs: runs on EVERY completion (including same-day
  --   duplicates) so finishing the current-day lesson always moves forward.
  -- Sprint (current prod): keeps the original rule — advance only on a NEW
  --   completion — so live production behavior is unchanged.
  -- Self-guarded by v_is_current_day_lesson, which is idempotent (advancing
  -- past a day means that day's lesson can no longer match current_program_day).
  -- ====================================================================
  select active_program_id, current_program_day
    into v_active_program_id, v_current_day
  from public.profiles
  where id = p_user_id;

  if v_active_program_id = v_sprint_program_id then
    -- Sprint: resolve the current-day lesson via program_schedule v1 (the
    -- live path), and the program length from the schedule row count.
    select exists (
      select 1
    ...
    ) into v_is_current_day_lesson;

    select count(*) into v_total_days
    from public.program_schedule
    where program_version = 'v1';
  elsif v_active_program_id is not null then
    -- Other packs: resolve via lessons.(program_id, sequence).
    select exists (
      select 1
      from public.lessons l
      where l.program_id = v_active_program_id
        and l.sequence = v_current_day
        and l.published = true
        and l.id = p_lesson_id
    ) into v_is_current_day_lesson;

    select coalesce(max(l.sequence), 0) into v_total_days
    from public.lessons l
    where l.program_id = v_active_program_id
      and l.published = true;
  end if;

  -- Advance gate: current-day lesson AND (non-Sprint OR a fresh completion).
  -- Sprint stays duplicate-gated (prod-safe); other packs always advance.
  if v_is_current_day_lesson
     and (v_active_program_id is distinct from v_sprint_program_id
          or not v_is_duplicate) then
    update public.profiles p
    set last_wod_completion_local_date = p_completion_local_date,
        current_program_day = least(p.current_program_day + 1, greatest(v_total_days, 1))
    where p.id = p_user_id
    returning p.current_program_day into v_new_day;

    -- Mirror the advanced day into per-pack state so a later Continue resumes
    -- here. Preserve the earliest started_local_date.
    insert into public.user_program_state
      (user_id, program_id, current_day, started, started_local_date)
    values
      (p_user_id, v_active_program_id, coalesce(v_new_day, v_current_day), true, p_completion_local_date)
    on conflict (user_id, program_id) do update set
      current_day        = excluded.current_day,
      started            = true,
      started_local_date = coalesce(public.user_program_state.started_local_date, excluded.started_local_date);
  end if;
