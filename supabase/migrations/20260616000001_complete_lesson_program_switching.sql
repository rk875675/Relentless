-- ============================================================================
-- 20260616000001_complete_lesson_program_switching.sql
-- Generalize complete_lesson to advance the user's ACTIVE program (any pack),
-- not just the hardcoded 30-Day Sprint. Same signature (uuid, uuid, date) and
-- same returned jsonb shape — only the program-advance block changes.
--
-- Sprint (active_program_id = b0000000-...-0001): resolves the current-day
--   lesson via program_schedule v1 exactly as before (live path unchanged).
-- Other packs: resolves via lessons.(program_id, sequence), capped at the
--   pack's own length (max published sequence).
--
-- Also mirrors the advanced day into user_program_state so a later switch can
-- Continue from where the user left off.
--
-- KEEP IN SYNC WITH supabase/functions/_shared/scoring.ts (gain steps / decay).
-- Based on 20260527000001_complete_lesson_atomic.sql.
-- ============================================================================
