-- ============================================================================
-- 20260712010000_complete_lesson_always_advance.sql
-- Make program-day auto-advance ALWAYS happen when the user completes their
-- current-day lesson — for every pack (Sprint + all coach packs), for all
-- current and future lessons.
--
-- Before: the whole program-advance block was gated on `not v_is_duplicate`,
--   so completing the current-day lesson a second time on the SAME calendar
--   day (e.g. after a Restart / pack switch that repointed you back to that
--   day) deduped and therefore never advanced to the next day.
--
-- After: the advance block runs unconditionally. It is still self-guarded by
--   `v_is_current_day_lesson` (lesson.sequence == profiles.current_program_day),
--   which is naturally idempotent: once you advance to day N+1, the day-N
--   lesson is no longer the current-day lesson, so it cannot double-advance.
--
-- MAC scoring and streaks remain gated on `not v_is_duplicate` (unchanged
-- anti-farming: same lesson, same local day = no extra % / streak). Only the
-- day pointer + user_program_state mirror move outside the duplicate guard.
--
-- Signature and returned jsonb shape are unchanged. Supersedes the advance
-- logic in 20260616000001_complete_lesson_program_switching.sql.
--
...
begin
  ...
  -- Runs on EVERY completion — including same-day duplicates — so finishing
  -- the current-day lesson always moves the pack forward. Self-guarded by
  -- v_is_current_day_lesson, which is idempotent (advancing past a day means
  -- that day's lesson can no longer match current_program_day).
  -- Generalized to the user's active program (any pack).
  -- ====================================================================
  ...
  if v_active_program_id = 'b0000000-0000-0000-0000-000000000001' then
  ...
  if v_is_current_day_lesson then
