...
const ONBOARDING_ROUTE_STEPS: Record<string, OnboardingStep> = {
  ...
  '/mac-framework': { step_key: 'mac_framework', step_index: ONBOARDING_PROGRESS.macFramework },
  '/mac-question': { step_key: 'mac_question', step_index: ONBOARDING_PROGRESS.macQuestion },
  '/mac-detail': { step_key: 'mac_detail', step_index: ONBOARDING_PROGRESS.macDetail },
  ...
  '/tutorial': { step_key: 'tutorial', step_index: 16 },
  ...
  // Older/alternate onboarding routes still in the app tree.
  ...
  '/mac-setup': { step_key: 'mac_setup', step_index: ONBOARDING_PROGRESS.macDetail },
...
export function useOnboardingStepTimer(resetKey: string): () => number {
  ...
  return useCallback(() => Date.now() - startedAtRef.current, []);
}
...
export function trackOnboardingSampleStarted(props: OnboardingEventProps): void {
  captureOnboardingEvent('onboarding_sample_started', props);
}

export function trackOnboardingSampleCompleted(props: OnboardingEventProps): void {
  captureOnboardingEvent('onboarding_sample_completed', props);
}
